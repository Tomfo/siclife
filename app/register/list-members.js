...
//from clerk
  ...
  const filteredList = useMemo(() => {
    ...
    return data.filter(record =>
      [record.firstName.trim(), record.lastName.trim(), record.email.trim(), record.middleName.trim()]
...
const deleteUser = useMutation({
    ...
    onSuccess: (_, id) => {
      queryClient.setQueryData(['getmembers'], old => old.filter(deleted => deleted.id !== id));
    }
  });
...
<paper  sx={{ width: '100%', overflow: 'hidden', padding: 2 }}>
			...
			<TableContainer
              ...
              <TableRow>
                <TableCell colSpan={6} align="center">	<Alert severity='info'>No Registered Members Found.</Alert></TableCell>
              </TableRow>
            )}
					</TableBody>
				</Table>
